...
# init HYPRE once (safe if already initialized)
try
...
data = ["serial" t_path t_setup t_sim (t_path+t_setup+t_sim) Threads.nthreads() ts]
mkpath("timings")
writedlm("timings/timings_serial.csv",
    ["mode" "t_path" "t_setup" "t_sim" "t_total" "nthreads" "timestamp";
     data], ',')
println("Timings saved: timings/timings_serial.csv")

# cleanup to encourage HYPRE finalizers
ws = nothing
states = nothing
case = nothing
GC.gc()
